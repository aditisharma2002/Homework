...
def get_orfs(iden, strand, seq):
	...
	if strand == '-' # this is the statement that will give us the anti-sense strand
		...
		for nucleotide in seq[::-1]: aseq += anti[nt]
		seq = aseq
	
	if i in range(len(seq)-3): #check for start codon and returning codons
		...
		if codon = 'ATG' # we need to get the protein sequence using a generic START codon. 
			...
			if pseq == 'None': # if the sequence cannot be translated, the computer moves on
				continue
			pseq = pseq.split('*')[0]
			start = i+len(pseq)*3+3 # this is the start of the ORF
			
			if len(pseq) < arg.l/3 or pseq == None: # this is checking the minimum length
				continue
...
for desc, seq in mcb.read_fasta(arg.file):
